...
exports.assign_image_to_multiple_users = async (req, res) => {
...
exports.delete_script = async (req, res) => {
    const {script_id} = req.body
    try {
        const script = await Media.destroy({where : {id : script_id}})
        return res.json(script)
    } catch (err) {
        console.log(err)
        return res.status(500).json(err)
...
exports.get_script_total_number = async (_, res) => {
    try {
        return res.json(await Media.count());
    } catch (err) {
        console.log(err)
        return res.status(500).json(err)
    }
...
exports.get_all_script_ids = async (_, res) => {
    try {
        return res.json(await Media.findAll({attributes: ['id']}));
    } catch (err) {
        console.log(err)
        return res.status(500).json(err)
    }
